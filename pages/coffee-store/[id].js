...
import { fetchCoffeeStores, fetcher } from "../../lib/coffee-stores";
...
const CoffeeStore = (initialProps) => {
    ...
    useEffect(() => {
        if(isEmpty(initialProps.coffeeStore)){
            if(coffeeStores.length > 0){
                const coffeeStoreFromContext = coffeeStores.find(store => store.id.toString() === id);
    ...
    const handleUpvoteButton = async () => {
        try {
            ...
            console.error("Error upvoting error store", err);
        }
    }
    ...
    if(error){
        return <h1>Something went wrong retrvieving coffee store page</h1>
    }
    ...
    return (
        <div className={styles.layout}>
            <Head>
                <title>{name}</title>
                <meta name={pageName} content="Generated by create next app" />
